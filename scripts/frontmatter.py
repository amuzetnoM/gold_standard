...
def add_frontmatter(
    content: str, filename: str, doc_type: Optional[str] = None, tags: Optional[List[str]] = None, **kwargs
) -> str:
    """
...
def parse_frontmatter(content: str) -> tuple[Dict[str, Any], str]:
    ...
    for line in frontmatter_str.split("\n"):
        if ":" in line:
            ...
            # Parse numbers (but not dates like 2025-12-04)
            elif value.replace(".", "").replace("-", "").isdigit() and "-" not in value:
...
def get_document_status(content: str) -> str:
    """
    Get the lifecycle status from document frontmatter.

    Returns:
        Status string ('draft', 'in_progress', 'published', etc.) or 'draft' if not set.
    """
    frontmatter, _ = parse_frontmatter(content)
    return frontmatter.get("status", "draft")


def set_document_status(content: str, status: str, filename: str = "document.md") -> str:
    """
    Update the lifecycle status in document frontmatter.
    ...
    if frontmatter:
        ...
        # No frontmatter, add it with status
        return add_frontmatter(content, filename, status=status)


def is_published(content: str) -> bool:
    """Check if document is published (ready for Notion sync)."""
...
def promote_status(content: str, filename: str = "document.md") -> str:
    """
    Promote document to next lifecycle status.

    draft -> in_progress -> review -> published
    """
    current = get_document_status(content)
    progression = {
        "draft": "in_progress",
        "in_progress": "review",
        "review": "published",
    ...
    new_status = progression.get(current, "draft")
    return set_document_status(content, new_status, filename)


# Convenience functions for specific report types
def journal_frontmatter(content: str, filename: str, bias: str = "NEUTRAL", gold_price: float = 0, **kwargs) -> str:
    """Generate frontmatter for journal entries."""
    return add_frontmatter(
        content, filename, doc_type="journal", custom_fields={"bias": bias, "gold_price": gold_price, **kwargs}
    )


def report_frontmatter(content: str, filename: str, report_type: str = "weekly", **kwargs) -> str:
    """Generate frontmatter for reports."""
    return add_frontmatter(content, filename, doc_type="reports", custom_fields={"report_type": report_type, **kwargs})
...
def chart_frontmatter(content: str, filename: str, ticker: str = "", **kwargs) -> str:
    ...
    return add_frontmatter(
        content, filename, doc_type="charts", tags=tags, custom_fields={"ticker": ticker.upper(), **kwargs}
    )
